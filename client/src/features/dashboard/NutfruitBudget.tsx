...
const MONTH_KEYS: (keyof NutfruitRow)[] = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'];
...
export default function NutfruitBudget({ year }: { year: number }) {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['nutfruit-budget', year],
        queryFn: () => adminApi.getNutfruitBudget(year),
        enabled: open,
        staleTime: 30_000,
    });
    ...
    const saveMutation = useMutation({
        mutationFn: adminApi.saveNutfruitCell,
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['nutfruit-budget', year] }),
        onError: () => toast.error('Error al guardar'),
    });

    const addRowMutation = useMutation({
        mutationFn: adminApi.addNutfruitRow,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['nutfruit-budget', year] });
            setNewRowName('');
            setAddingRow(false);
    ...
    const deleteRowMutation = useMutation({
        mutationFn: adminApi.deleteNutfruitRow,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['nutfruit-budget', year] });
    ...
    const startEdit = (row: NutfruitRow, monthIdx: number) => {
        if (!canEdit) return;
    ...
    const rowTotal = (row: NutfruitRow) =>
        MONTH_KEYS.reduce((s, k) => s + (Number(row[k]) || 0), 0);

    const sectionTotal = (section: NutfruitRow[]) =>
        MONTH_KEYS.map((_, i) =>
    ...
    const renderCell = (row: NutfruitRow, mIdx: number) => {
    ...
    return (
        <div className="rounded-2xl border border-amber-200 overflow-hidden">
            ...
            <button
                onClick={() => setOpen(o => !o)}
                className="w-full flex items-center justify-between px-5 py-3.5 bg-amber-50 hover:bg-amber-100/60 transition-colors"
            >
